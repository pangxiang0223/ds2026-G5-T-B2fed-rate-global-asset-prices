...
## ⚠️ 注意事项
1. **网络环境限制**：使用 yfinance 挖掘金融数据对网络环境要求较高，需要稳定的 VPN 连接才能稳定获取海外金融数据
2. **中文乱码问题**：matplotlib 默认字体不支持中文字符，需要配置中文字体（如 SimHei、Microsoft YaHei）以避免图表中文显示为方框
...
#### 可视化结果
![资产β系数对比图](output/asset_beta_comparison.png)
#### 关键数据
| 资产       | β系数   | 含义         |
|------------|---------|--------------|
| S&P 500    | +0.0133 | 正相关       |
...
##### 各类资产分析
1.  **标普500**
    正向利率敏感，β为0.0133。本轮利率上行阶段多对应宏观经济基本面修复阶段，企业盈利改善带来的正向拉动，对冲了利率抬升带来的折现率负面压制。标普500成分股覆盖大量周期行业与价值类上市公司，在经济回暖周期盈利释放能力更强，最终形成对利率上行的正向收益反馈，正向利率敏感度在权益资产中处于较高水平。

2.  **纳斯达克100**
    弱正向利率敏感，β为0.0086，低于标普500。由于纳斯达克100以科技成长标的为主，资产估值更依赖远期现金流折现，利率上行带来的估值收缩更为突出。经济复苏带动盈利提升的正向拉动弱于标普500周期板块，因此表现出更弱的正向利率敏感性。

3.  **20Y 美债**
    20Y 美债：负向利率敏感，β为‑0.0255，契合债券久期定价理论，长债现金流回收周期长，久期水平高，利率上行会直接抬升市场贴现，引发债券市场价格下行。利率上行幅度越大，长期美债收益下行幅度越高，是固收资产中利率风险的承载品。
4.  **黄金**
    弱负向利率敏感，β为‑0.0122，利率上行将拖累黄金资产收益。黄金属于无息避险资产，无利息收益，持有机会成本挂钩无风险利率。在货币政策收紧、利率上行阶段，市场资金会转向国债、现金等生息资产，配置黄金的意愿回落，形成利率抬升、黄金收益下行的负相关关系。负相关强度弱于长期美债。

5.  **比特币**
    强负相关利率敏感，β为‑0.0914，在全部资产中与利率的负相关度最高。一方面，比特币不存在经营性现金流与固定收益，资产定价完全依托市场流动性与远期风险偏好折现，利率上行会直接收缩市场整体流动性，压低比特币估值水平；另一方面，比特币属于高波动风险资产，在加息周期下，市场风险偏好下行，资本持续撤离风险资产，利率上行与比特币收益下行形成极强的负相关关系。
##### 如果预期加息，你会怎么配置资产？
第一层：确定性减仓固定收益产品。 长期美债是唯一在加息周期中录得负累计收益且最大回撤达38%的资产，其久期逻辑、周期收益数据、实际回撤幅度三重证据一致指向同一结论：加息环境下降低长久期固收敞口是优先操作。
第二层：核心配置宽基指数、黄金。 SP500（β几乎为零，加息期月均+0.6%，累计+22%）和Gold_ETF（β接近零，最大回撤仅12%，累计+28%）在加息周期中展现了韧性。一方面，权益端保留SP500的宽基配置，捕捉盈利增长对估值压缩的对冲。另一方面，黄金的低回撤高累计收益特征使其成为组合中的稳定器，尤其在央行购金需求、主权信用危机形成结构性支撑的背景下，黄金对利率的敏感性正在被非利率因子持续稀释。
第三层：审慎对待纳斯达克、比特币。 Nasdaq100在加息期月均收益+1.3%，表现甚至优于降息期，但回撤达28%，波动成本不低。Bitcoin的β为-0.0914看似构成加息对冲工具，但如前述分析，样本不足一轮半周期、定价结构持续漂移、因果归因不清，这个β不具备配置决策的参考价值。对这两类资产，不是不能配，而是仓位控制和风险预算的设定需要脱离β框架，回到对基本面和流动性条件的独立判断。
